...
- **位置与解锁**：一言卡片（`.quote-corner`）未签到时是封签面板 `#checkinPanel`（朱印+提示+「翻开今日签」），签到后隐藏并解锁 `#quoteBody`——**一言此时才敲 hitokoto**（`initQuote` 里 `isCheckedInToday()` 把关），未签到不浪费外部请求。「换一句」随时可刷；hitokoto 对同 IP 短窗缓存，**请求必须带 `_t` nonce** 否则刷新拿回同一句。签面上的文字（吉凶/签句）走每日固定种子，不跟一言刷新联动。
- **状态存储**：`localStorage` 的 `checkin:YYYY-MM-DD`（当日已签）、`checkinLast` + `checkinStreak`（连签：昨天签过 +1、断了重计）；隐私模式写失败不阻断出签。
- **签面确定性**：日期字符串过 FNV-1a 出种子、mulberry32 出序列，**同一天任何时刻同一支签**；吉凶按 `LUCK_POOL` 权重抽（合计 100：大吉16/中吉20/小吉16/吉16/末吉12/凶12/大凶8），签句从 `SIGN_SENTENCES` 抽。
...
- **签到符画布**：`drawTalisman` 画 380×680 逻辑像素、2x 导出，**网易云日签海报式**：全幅生成图打底（`drawTalismanScene`：种子从 `SCENE_PALETTES` 六套手工配色盘选一套，平滑贝塞尔山脊四层+层间两端渐隐雾带拉空气透视，日月带径向光晕且约束在左半区避免压签句，左半天空两三只飞鸟，种子 45% 概率把最下层换成湖面+倒影光斑（**水色取中天色相压暗**，用地平暖色相会在暗色下跳成一整块芥末黄），上下再压暗角渐变保白字可读；左上小字公历+星期、大字农历（毛笔）、节日小字；**吉凶居中竖写**毛笔大字；右起竖排毛笔签句（**均分列长**不留孤字尾列、列首避开句读，最多三列）；左侧竖题头「拾光签」；**底部完全不放文字**（吉凶仅居中竖写一处；宜忌胶囊/信息 caption/朱印/左下标注均已下掉，`drawPill` 删除），留给画面本身。文字统一马善政毛笔楷书（appearance.js 的 `loadBrushFont` 画签前按需拉 `@fontsource/ma-shan-zheng@5` 并用签面样串触发 unicode-range 分片，拉不动回退站点字体）；标点走 `VERTICAL_PUNCT` 竖式字形；公历小字用阿拉伯数字，大字为农历汉写（八月十五），签面其余处不摆数字。弹层外壳透明当分享页（`.talisman-box` 无底无边，画布圆角+投影，按钮半透白）。明暗主题各一套天色/朱印色。
