...
├── scripts/              # 同步脚本（sync_tmdb.py / sync_stars.py）
└── .github/workflows/    # 定时任务（TMDB 评分 / GitHub Stars 每日同步）
```
...
`tracks[]: {title, artist, src}`；`src` 支持本地（放 `assets/music/`）或外链。音频缺失时标题会降级提示"音频加载失败"。
...
6. 窗口 resize：跨 768px 断点收抽屉、重算 marquee
...
- 两个工作流都用 Actions 的 git 身份提交——**本地 push 遇到 `[rejected] fetch first` 时先 `git pull --rebase` 再推**（就是它们的新提交）
