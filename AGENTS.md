...
- **签到符画布**：`drawTalisman` 画 380×680 逻辑像素、2x 导出，**网易云日签海报式**：全幅生成图打底（`drawTalismanScene`：种子从 `SCENE_PALETTES` 六套手工配色盘选一套，平滑贝塞尔山脊四层+层间两端渐隐雾带拉空气透视，日月带径向光晕且约束在左半区避免压签句，左半天空两三只飞鸟，种子 45% 概率把最下层换成湖面+倒影光斑（**水色取中天色相压暗**，用地平暖色相会在暗色下跳成一整块芥末黄），上下再压暗角渐变保白字可读；左上大日期数字+小年月+星期/节日，**吉凶居中竖写**（毛笔大字+列尾一枚小朱印当落款），右起竖排毛笔签句（换列最多三列），左侧竖题头「拾光签」；**底部不放任何文字**（宜忌胶囊/信息 caption 已按用户要求下掉，`drawPill` 一并删除），留给画面本身。文字统一马善政毛笔楷书（appearance.js 的 `loadBrushFont` 画签前按需拉 `@fontsource/ma-shan-zheng@5` 并用签面样串触发 unicode-range 分片，拉不动回退站点字体）；标点走 `VERTICAL_PUNCT` 竖式字形；海报日期大数字用阿拉伯数字（网易云海报惯例），其余签面不摆数字。弹层外壳透明当分享页（`.talisman-box` 无底无边，画布圆角+投影，按钮半透白）。明暗主题各一套天色/朱印色。
