...
- **位置与解锁**：一言卡片（`.quote-corner`）未签到时是封签面板 `#checkinPanel`（朱印+提示+「翻开今日签」），签到后隐藏并解锁 `#quoteBody`——**一言此时才敲 hitokoto**（`initQuote` 里 `isCheckedInToday()` 把关），未签到不浪费外部请求。「换一句」随时可刷；hitokoto 对同 IP 短窗缓存，**请求必须带 `_t` nonce** 否则刷新拿回同一句。签面上的文字（吉凶/宜忌/签句/信息）走每日固定种子，不跟一言刷新联动。
...
- **签到符画布**：`drawTalisman` 画 380×680 逻辑像素、2x 导出，**网易云日签海报式**：全幅生成图打底（`drawTalismanScene`：种子从 `SCENE_PALETTES` 六套手工配色盘选一套，平滑贝塞尔山脊四层+层间两端渐隐雾带拉空气透视，日月带径向光晕且约束在左半区避免压签句，左半天空两三只飞鸟，种子 45% 概率把最下层换成湖面+倒影光斑，上下再压暗角渐变保白字可读；左上大日期数字+小年月+星期/节日，右上吉凶竖印，右起竖排毛笔签句（换列最多三列），左侧竖题头「拾光签」，宜忌半透白胶囊（`drawPill`），底部信息 caption 两行（农历干支/年第N天/连签），右下品牌朱印。文字统一马善政毛笔楷书（appearance.js 的 `loadBrushFont` 画签前按需拉 `@fontsource/ma-shan-zheng@5` 并用签面样串触发 unicode-range 分片，拉不动回退站点字体）；标点走 `VERTICAL_PUNCT` 竖式字形、签面数字汉写（`yearToCn`/`numToCn`），但海报日期大数字用阿拉伯数字（网易云海报惯例）。弹层外壳透明当分享页（`.talisman-box` 无底无边，画布圆角+投影，按钮半透白）。明暗主题各一套天色/朱印色。
